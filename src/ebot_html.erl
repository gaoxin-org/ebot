...
-record(state,{
	  workers = ebot_worker_util:create_workers(?WORKER_TYPE)       %%return : {Type, []}
	 }).
...
analyze_url_head(Url, _Result = {_Status, Headers, empty}) ->
    {ok, H} = ebot_util:get_env(tobe_saved_headers),
    Options = [{head, Headers, H}, 
...
analyze_url_body_links(Url, Links) ->
    ...
    error_logger:info_report({?MODULE, ?LINE, {not_visited_links, NotVisitedLinks}}),
    %% retrive Url from DB
    lists:foreach(
      fun(U) ->
          ...
          DB_Ret = ebot_db:open_or_create_url(U),
...
analyze_url_body_plugin(Url, Tokens, Module, Function) ->  
    ...
    error_logger:info_report({?MODULE, ?LINE, {analyze_url_body_plugin, Url, Module, Function}}),
    ebot_db:update_doc(Url, Options).
